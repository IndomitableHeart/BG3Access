...
local function UpdateTrackingState(playerPosition)
    ...
    if currentPath then
        ...
        if pathWasAvailable or not noPathAnnounced then
            ...
            if not noPathAnnounced then
                Ext.Tolk.Speak(
                    "No path to " .. trackingTarget.name, true)
    ...
    if hazardAhead then
        ...
        if not lastHazardAnnouncedLabel then
            ...
            Ext.Tolk.Speak(
                "Stop. " .. hazardText .. " "
                    .. distanceRounded .. " meters ahead", true)
        end
    ...
    for _, entry in ipairs(allEntities) do
        ...
        if distance <= GPS_PROXIMITY_TIER1_ENTER_M then
            ...
            if not tier1Latched[entry.entityKey] then
                ...
                Ext.Tolk.Speak(entry.name, false)
            end
...
function EnterExplorationMode(introText)
    ...
    Ext.Tolk.Speak(introText or "Exploration mode", true)
...
local function StartTracking(targetEntry)
    ...
    if not currentPath then
        ...
        Ext.Tolk.Speak("Tracking " .. trackingTarget.name .. ". "
            .. distanceRounded .. " meters" .. directionText
            .. hazardText, true)
        Log.Info("GPS: Tracking " .. trackingTarget.name
            .. " (" .. distanceRounded .. "m, "
            .. #currentPath .. " nodes"
...
local function AnnounceCategorySwitch(playerPosition, prefix)
    ...
    if #entities == 0 then
        ...
        Ext.Tolk.Speak(
            prefixText .. categoryName .. ". None.", true)
    else
        ...
        Log.Info("List [" .. categoryName .. " 1/"
            .. #entities .. "]: " .. itemSpeech)
        -- Single combined speech: "prefix. Category. First item,
        -- N meters."  Keeps the full announcement on one
        -- interrupt so Tolk does not chop the category line.
        Ext.Tolk.Speak(
            prefixText .. categoryName .. ". " .. itemSpeech,
            true)
    end
end
                ...
                if progress >= GPS_PROGRESS_DELTA then
                    ...
                    if stuckTickCount >= GPS_STUCK_TICKS
                        ...
                        Log.Info(string.format(
                            ...
                            stuckTickCount,
                            currentDistanceToTarget))
                        Ext.Tolk.Speak("Path blocked", true)
